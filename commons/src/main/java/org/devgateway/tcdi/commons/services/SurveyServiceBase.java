package org.devgateway.tcdi.commons.services;
...
import org.devgateway.tcdi.commons.domain.Dimension;
import org.devgateway.tcdi.commons.domain.categories.Category;
import org.devgateway.tcdi.commons.pojo.Response;
...
public abstract class SurveyServiceBase<R extends JpaRepository, Q extends EntityPathBase, S> {
    ...
    protected R surveyRepository;
    protected Q qSurvey;
    protected Class<S> surveyType;

    @Autowired
    public SurveyServiceBase(final R surveyRepository, Q qSurvey, Class<S> surveyType) {
                ...
                if (f != null) {
                    if (f.getType().getGenericSuperclass().getTypeName().equalsIgnoreCase(Category.class.getTypeName())) {
                        builder.and(Expressions.predicate(Ops.IN,
                                ...
                                Expressions.constant(Arrays.stream(params.get(key).split(","))
                                        .map(o -> Long.parseLong(o)).collect(Collectors.toList()))));
    ...
    public Dimension getDimensionByName(String name) {
        for (Field field : surveyType.getDeclaredFields()) {
    ...
    public List getFilters() {
        ...
        for (Field field : surveyType.getDeclaredFields()) {
            if (field.isAnnotationPresent(org.devgateway.tcdi.commons.domain.Filtrable.class)) {
                org.devgateway.tcdi.commons.domain.Filtrable[] d;
                d = field.getAnnotationsByType(org.devgateway.tcdi.commons.domain.Filtrable.class);
    ...
    public abstract Expression sumExpression();


    public Tuple sum() {
        JPAQuery query = new JPAQuery<>(em);
        query.select(sumExpression(), qSurvey.count()).from(qSurvey);
    ...
    public List<Tuple> sum(Map<String, String> params, List<String> dimensions) {
        JPAQuery query = new JPAQuery<>(em);
        BooleanBuilder builder = getFilters(params);
        List<Expression> expressions = dimensions.stream().map(s -> gePathByDimension(s)).collect(Collectors.toList());
        if (expressions.size() > 0) {
            List<Expression> select = new ArrayList<>(expressions);
            List<Expression> group = new ArrayList<>(expressions);
            select.add(sumExpression());
    ...
    public S save(final S element) {
        return (S) this.surveyRepository.save(element);
    }
